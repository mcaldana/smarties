...
DataCoordinator::DataCoordinator(MemoryBuffer*const RM, ParameterBlob & P)
  : replay(RM), params(P)
{
  episodes.reserve(m_ExecutionInfo.nAgents);

  sharingComm = MPICommDup(m_ExecutionInfo.workerless_masters_comm);
  sharingSize = MPICommSize(sharingComm);
  sharingRank = MPICommRank(sharingComm);

  if(m_ExecutionInfo.learnersOnWorkers &&
     m_ExecutionInfo.nOwnedEnvironments &&
     m_ExecutionInfo.master_workers_comm != MPI_COMM_NULL)
  {
    workerComm = MPICommDup(m_ExecutionInfo.master_workers_comm);
    ...
    if(workerRank == 0) {
      if(!m_ExecutionInfo.bIsMaster) die("impossible");
    } else {
      if(m_ExecutionInfo.bIsMaster) die("impossible");
    }
  } else {
  ...
  if(m_ExecutionInfo.workerless_masters_comm != MPI_COMM_NULL)
  {
...
{
  if(sharingComm != MPI_COMM_NULL) MPI_Comm_free(&sharingComm);
...
void DataCoordinator::distributePendingEpisodes()
{
  ...
  while ( episodes.size() )
  {
    ...
    else
    {
      ...
      if( sharingReq[dest] != MPI_REQUEST_NULL)
                                  ...
                                  const bool bUpdateParams)
{
  ...
  {
    assert(m_ExecutionInfo.bIsMaster);
  ...
  else if(bRunParameterServer)
  {
    ...
    assert(workerRank>0 && workerSize>1 && !m_ExecutionInfo.bIsMaster);
